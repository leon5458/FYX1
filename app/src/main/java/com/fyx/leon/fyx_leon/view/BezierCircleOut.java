...
import com.fyx.leon.fyx_leon.utils.bezierutils.HPoint;
import com.fyx.leon.fyx_leon.utils.bezierutils.VPoint;
 ...
 * <p>版本号：1<p>
 */
public class BezierCircleOut extends View {
    /** 路径 */
    private Path mPath;
    /** 画笔*/
    ...
    private float stretchDistance;//改变圆的距离

    public BezierCircleOut(Context context) {
        this(context, null);
    }

    public BezierCircleOut(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BezierCircleOut(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        mFillCirclePaint.setColor(0xff0babfe);//fe626d);#0babfe
...
//       canvas.translate(radius + 10, getHeight() / 2);
        canvas.translate(0 , getHeight() / 2);// 这里是改变绘图的位置
        ...
        CircleModelBig(canvas);
    }

    private void CircleModelBig(Canvas canvas) {
    ...
    private void CircleSpecLast(float time) {//0~0.2
        CircleModel();
        p2.setX(radius + stretchDistance * time * 8); // 改变最右边的点x
    }
    ...
    public void startAnimation() {
        mPath.reset();
        mInterpolatedTime = 0;
        BezierCircleOut.MoveAnimation move = new BezierCircleOut.MoveAnimation();
